...
export const MachineProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const channel = supabase.channel('schema-db-changes')
      .on(
        ...
        (payload) => {
          ...
          // (Poderíamos otimizar atualizando apenas o item alterado, mas reload é mais seguro para consistência)
          if (payload.eventType === 'UPDATE') {
             // Mapeamento rápido para update otimista ou recarga
             // Por segurança, chamamos loadData para garantir sincronia total
             loadData(); 
          } else {
             loadData();
          }
  ...
  const updateMachineStatus = async (id: number, status: MachineStatus, reason: StopReason | null = null, notes: string = '', operator: string = 'Operador Turno A', signature?: string) => {
    ...
    try {
      ...
      // O Realtime atualizará o history automaticamente para todos
    } catch (e) {
  ...
  useEffect(() => {
    ...
    const checkReset = async () => {
      ...
      if (now >= shiftStartDate) {
        try {
          // EXCEL DOWNLOAD AUTOMÁTICO ANTES DO RESET (Apenas no cliente que disparar)
          // Isso é tricky em multi-client, idealmente o backend faria isso, mas mantemos no front por enquanto.
          if (!lastResetDate) {
             // Evita download no primeiro load limpo
          }
  ...
  const generateMockData = () => {
    showNotification("Gerando dados de teste (Modo Supabase)...", 'info');
  };
  ...
  const importData = (json: string): boolean => {
    // Importação local desativada em favor do banco de dados para consistência, 
    // ou precisaria implementar um bulk insert no Supabase.
    showNotification("Importação via JSON desativada no modo Online.", 'warning');
    return false;
  };
